...
//		employ2.srednInMonth();
		...
		// Вывод для проверки что вначале 
		 for (int i = 0; i < array.length; i++)
		 {
			 System.out.println("Admin - name = " + array[i].name + " money = " + array[i].money);
		 }
		 System.out.println("\n");
		 // Упорядочиваю по зарплате, если они одинаковые вы сказали смотреть их по именам ( я специально сделал все зп одинаковымы чтобы проверить сортировку по именам), я сидел и втыкал почему не работает а в итоге я понял что имена то я на английском написал алфавит юзал русский))))))))))
		 ...
		 for (int i = 0; i < array.length; i++) {
		    for (int j = 1; j < array.length; j++) {
		    	...
		    	if ((int)array[j-1].money == (int)array[j].money)
			    {
		    		...
		    		//System.out.println("iteration " + (j - 1 + " first = " + first + " second = " + second)); // Для проверки
		    		if (first == second)
		    		{
		    			break;
		    		}
		...
		// Вывод для проверки что получилось после сортировки 
		 for (int i = 0; i < array.length; i++)
		 {
			 System.out.println("Admin - name = " + array[i].name + " money = " + array[i].money); // Выведет правильно по алфавиту (a b d g v)
		 }
		 
//		 System.out.println("Первые 5: ");
//		 try(FileWriter writer = new FileWriter("test_4.txt", false))
//	        {
//			// Печатаю для проверки
//			 for (int i = 0; i < 5; i++) {
