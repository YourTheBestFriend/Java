...
public class class_laba_6 {
	...
	// 10.2
	public static ArrayList<Integer> find_palindromes(String x) // поиск чисел
	{
		...
		for(int i = 0; i < words.length; i++)
		{
			if (words[i].matches("[0-9]+")) // проверка на регулярку ( + это от 1 и больше элементов того что слева)
			{
				...
				if (is_palindrome(x_znach)) // проверка его на палиндром
				{
	...
	public static void main(String[] args) {
		...
		*/
		...
		кое что воял в итоге внизу все одной функцией
		...
		*/
		 ...
		 */
		int wordIndex = (int)words[0].chars().distinct().count(); // индекс первого слова с кол-вом подсчета не повторающихся символов
